...
def catsdogs_get_datasets(data, load_train=True, load_test=True):
    ...
    (data_dir, args) = data
    path = data_dir
    dataset_path = os.path.join(path, "cars_vs_non")
    is_dir = os.path.isdir(dataset_path)
    if not is_dir:
        path = os.getcwd()
        dataset_path = os.path.join(path, "cars-vs-not")
        is_dir = os.path.isdir(dataset_path)
        ...
        if not is_dir:
            ...
            path = os.getcwd()
            dataset_path = os.path.join(path, "cars-vs-not", "train")
            is_dir = os.path.isdir(dataset_path)
            if not is_dir:
                sys.exit("Unzip \'train.zip\' file from dogs-vs-cats directory")

            # create directories
            dataset_home = os.path.join(data_dir, "cars_vs_non")
            newdir = os.path.join(dataset_home, "train", "car")
            makedirs(newdir, exist_ok=True)
            newdir = os.path.join(dataset_home, "train", "non")
            makedirs(newdir, exist_ok=True)
            newdir = os.path.join(dataset_home, "test", "car")
            makedirs(newdir, exist_ok=True)
            newdir = os.path.join(dataset_home, "test", "non")
            makedirs(newdir, exist_ok=True)

            # define ratio of pictures to use for test set
            test_ratio = 0.2
            # copy training dataset images into subdirectories
            src_directory = os.path.join(path, "cars-vs-not", "train")
            for file in listdir(src_directory):
                ...
                if file.startswith("car"):
                    dst = os.path.join(dst_dir, "car", file)
                    shutil.copyfile(src, dst)
                elif file.startswith("non"):
                    dst = os.path.join(dst_dir, "non", file)
                    shutil.copyfile(src, dst)
            shutil.rmtree("cars-vs-not")

    training_data_path = os.path.join(data_dir, "cars_vs_non")
    training_data_path = os.path.join(training_data_path, "train")

    test_data_path = os.path.join(data_dir, "cars_vs_non")
